...
try {
    ...
    if (!canCurrentUserAccessDashboard($access_pdo, '../app/SYSTEM/index')) {
        header("Location: ../../auth/login");
        exit();
    }
    ...
    if (shouldRequireBranchSelection($access_pdo)) {
        ...
        if ($active_branch_id === '' || !setCurrentBranchContext($access_pdo, $active_branch_id)) {
            header('Location: ../../auth/branch_selector_popup.php?redirect=' . rawurlencode('../app/SYSTEM/index'));
            exit();
        }
    }
} catch (Throwable $e) {
    error_log('Role access check failed in SYSTEM/index.php: ' . $e->getMessage());
    header("Location: ../../auth/login");
    exit();
}
...
$profile_role = (string)($_SESSION['user_role'] ?? 'system_admin');
$profile_image_src = '';
...
<?php echo renderSharedTopbar([
    ...
    'brand_sub' => 'Technical Management System',
