...
try {
    ...
    if (!canCurrentUserAccessDashboard($access_pdo, '../app/sell/pagesell')) {
        header("Location: ../../auth/login");
        exit();
    }
    ...
    if (shouldRequireBranchSelection($access_pdo)) {
        ...
        if ($active_branch_id === '' || !setCurrentBranchContext($access_pdo, $active_branch_id)) {
            header('Location: ../../auth/branch_selector_popup.php?redirect=' . rawurlencode('../app/sell/pagesell'));
            exit();
        }
    }
} catch (Throwable $e) {
    error_log('Role access check failed in sell/pagesell.php: ' . $e->getMessage());
    header("Location: ../../auth/login");
    exit();
}

$admin_display_name = 'Sell Car';
$nav_logo_src = '';
...
$profile_position = (string)($_SESSION['position'] ?? 'Sell Car');
$profile_role = (string)($_SESSION['user_role'] ?? 'sell_car');
$profile_image_src = '';

$group_box_has_membership = false;
$group_box_group_name = 'ยังไม่ได้ Join Group Sales';
$group_box_invite_code = '-';
$group_box_manager_name = '-';
$group_box_member_total = 0;
$group_box_member_active = 0;
$group_box_member_status = 'inactive';
$group_box_team_status = 'inactive';
$group_box_joined_at_display = '';
$group_box_updated_at_display = '';

if (!function_exists('formatSellDashboardDateTime')) {
    function formatSellDashboardDateTime($value) {
...
try {
    ...
    $active_branch_for_group = trim((string)($_SESSION['active_branch_id'] ?? ''));
    if ($active_branch_for_group !== '' && $profile_user_id !== '') {
        $has_group_members_table = false;
        $has_group_invites_table = false;

        $members_table_stmt = $pdo->query("SHOW TABLES LIKE 'sales_group_members'");
        if ($members_table_stmt && $members_table_stmt->fetch()) {
            $has_group_members_table = true;
        }

        $invites_table_stmt = $pdo->query("SHOW TABLES LIKE 'sales_group_invites'");
        if ($invites_table_stmt && $invites_table_stmt->fetch()) {
            $has_group_invites_table = true;
        }

        if ($has_group_members_table && $has_group_invites_table) {
            ...
            $group_stmt->execute([$profile_user_id, $active_branch_for_group]);
            $group_row = $group_stmt->fetch(PDO::FETCH_ASSOC);

            if ($group_row) {
                $manager_name = trim(
                    (string)($group_row['manager_first_name'] ?? '') .
                    ' ' .
                    (string)($group_row['manager_last_name'] ?? '')
                );
                if ($manager_name === '') {
                    $manager_name = trim((string)($group_row['manager_user_id'] ?? '-'));
                }

                $group_box_has_membership = true;
                $group_box_group_name = (string)($group_row['group_name'] ?? $group_box_group_name);
                $group_box_invite_code = (string)($group_row['invite_code'] ?? $group_box_invite_code);
                $group_box_manager_name = $manager_name !== '' ? $manager_name : '-';
                $group_box_member_total = max(0, (int)($group_row['total_member_count'] ?? 0));
                $group_box_member_active = max(0, (int)($group_row['active_member_count'] ?? 0));

                $member_status = strtolower(trim((string)($group_row['member_status'] ?? 'active')));
                $team_status = strtolower(trim((string)($group_row['group_status'] ?? 'active')));
                $group_box_member_status = in_array($member_status, ['pending', 'active', 'suspended'], true) ? $member_status : 'inactive';
                $group_box_team_status = in_array($team_status, ['active', 'suspended'], true) ? $team_status : 'inactive';

                $group_box_joined_at_display = formatSellDashboardDateTime($group_row['created_at'] ?? '');
                $group_box_updated_at_display = formatSellDashboardDateTime($group_row['updated_at'] ?? '');
            }
        }
    }
} catch (PDOException $e) {
    error_log('Failed to load admin display name in sell/pagesell.php: ' . $e->getMessage());
}
...
$group_status_label_map = [
    'pending' => 'รออนุมัติ',
    'active' => 'ใช้งาน',
    'suspended' => 'ระงับ',
    'inactive' => 'ยังไม่เข้าร่วม'
];
$group_box_member_status_label = $group_status_label_map[$group_box_member_status] ?? 'ไม่ทราบสถานะ';
$group_box_team_status_label = $group_status_label_map[$group_box_team_status] ?? 'ไม่ทราบสถานะ';

$group_box_member_status_class = in_array($group_box_member_status, ['pending', 'active', 'suspended'], true)
    ? $group_box_member_status
    : 'inactive';
$group_box_team_status_class = in_array($group_box_team_status, ['active', 'suspended'], true)
    ? $group_box_team_status
    : 'inactive';
$group_box_member_count_text = $group_box_has_membership
    ? number_format($group_box_member_active) . '/' . number_format($group_box_member_total) . ' คน'
    : '-';
...
$menu_role_config = getDashboardMenuConfigByRole(
    ...
    [
        'home_href' => 'pagesell.php',
        'page_title' => 'Sell Car Dashboard',
        'portal_label' => 'Office Plus ERP - Sell Car Portal'
    ]
);
...
@media (max-width: 1280px) {
    .group-box-name { font-size: 20px; }
    .group-box-empty { font-size: 15px; }
    .group-box-row { font-size: 13px; }
    .group-box-code { font-size: 12px; min-height: 25px; }
    .group-badge { height: 28px; font-size: 12px; padding: 0 11px; }
    .group-box-member-count { font-size: 13px; }
}

@media (max-width: 900px) {
    .group-box-name { font-size: 18px; }
    .group-box-empty { font-size: 14px; }
    .group-box-row { font-size: 12px; }
    .group-box-code { font-size: 11px; min-height: 24px; }
    .group-badge { height: 26px; font-size: 11px; padding: 0 9px; }
    .group-box-member-count { font-size: 12px; }
}
...
<?php echo renderSharedTopbar([
    ...
    'brand_sub' => 'Sell Car Management System',
            ...
            <div class="d-row">
                ...
                <div class="card kpi-card anim-in" style="animation-delay:.08s;">
                    <div class="card-head">
                        ...
                        <span class="card-title">Group Sales</span>
                    </div>
                    <div class="kpi-inner">
                        <div class="kpi-lbl">Sales Team</div>
                        <?php if ($group_box_has_membership): ?>
                            <div class="group-box-name"><?php echo htmlspecialchars($group_box_group_name); ?></div>
                            <div class="group-box-row">
                                Invite Code:
                                <span class="group-box-code"><?php echo htmlspecialchars($group_box_invite_code); ?></span>
                            </div>
                            <div class="group-box-row">หัวหน้าทีม : <strong><?php echo htmlspecialchars($group_box_manager_name); ?></strong></div>
                            <div class="group-box-badges">
                                <span class="group-badge <?php echo htmlspecialchars($group_box_member_status_class); ?>">Member: <?php echo htmlspecialchars($group_box_member_status_label); ?></span>
                                <span class="group-badge <?php echo htmlspecialchars($group_box_team_status_class); ?>">Team: <?php echo htmlspecialchars($group_box_team_status_label); ?></span>
                            </div>
                            <?php if ($group_box_member_status === 'pending'): ?>
                                <div class="group-box-row">สถานะคำขอ : รอหัวหน้าทีมอนุมัติ</div>
                            <?php endif; ?>
                            <div class="group-box-member-count">สมาชิกทีม (ใช้งาน/ทั้งหมด) : <strong><?php echo htmlspecialchars($group_box_member_count_text); ?></strong></div>
                            <div class="group-box-row"><?php echo $group_box_member_status === 'pending' ? 'ส่งคำขอเมื่อ' : 'Join เมื่อ'; ?> : <?php echo htmlspecialchars($group_box_joined_at_display !== '' ? $group_box_joined_at_display : '-'); ?></div>
                            <div class="group-box-row">อัปเดตล่าสุด : <?php echo htmlspecialchars($group_box_updated_at_display !== '' ? $group_box_updated_at_display : '-'); ?></div>
                        <?php else: ?>
                            <div class="group-box-empty">ยังไม่ได้ Join Group Sales</div>
                            <div class="kpi-sub">กรอกรหัสเชิญเพื่อเข้าร่วมทีมฝ่ายขาย</div>
                            <div class="group-box-badges">
                                <span class="group-badge inactive">Member: ยังไม่เข้าร่วม</span>
                            </div>
                            <a class="group-box-link" href="join_group_sales.php?module=groupsjoin">ไปหน้า Join Group</a>
                        <?php endif; ?>
                    </div>
                </div>
