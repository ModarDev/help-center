...
try {
    ...
    if (!canCurrentUserAccessDashboard($access_pdo, '../app/sales_manager/page_sell_manager')) {
        header("Location: ../../auth/login");
        exit();
    }
    ...
    if (shouldRequireBranchSelection($access_pdo)) {
        ...
        if ($active_branch_id === '' || !setCurrentBranchContext($access_pdo, $active_branch_id)) {
            header('Location: ../../auth/branch_selector_popup.php?redirect=' . rawurlencode('../app/sales_manager/page_sell_manager'));
            exit();
        }
    }
} catch (Throwable $e) {
...
$admin_display_name = 'sales_manager';
$nav_logo_src = '';
...
$profile_position = (string)($_SESSION['position'] ?? 'Sales Manager');
$profile_role = (string)($_SESSION['user_role'] ?? 'sales_manager');
$profile_image_src = '';

$group_box_has_group = false;
$group_box_group_name = 'ยังไม่มีทีมขาย';
$group_box_invite_code = '-';
$group_box_member_total = 0;
$group_box_member_active = 0;
$group_box_member_pending = 0;
$group_box_group_total = 0;
$group_box_team_status = 'inactive';
$group_box_created_at_display = '';
$group_box_updated_at_display = '';
$group_box_focus_group_id = 0;

if (!function_exists('formatSalesManagerDashboardDateTime')) {
    function formatSalesManagerDashboardDateTime($value) {
        ...
        if ($has_group_members_table && $has_group_invites_table) {
            $group_stmt = $pdo->prepare(
                'SELECT
                    ...
                    (
                        SELECT COUNT(*)
                        FROM sales_group_invites sx
                        WHERE sx.manager_user_id = g.manager_user_id
                          AND sx.branch_id = g.branch_id
                    ) AS total_group_count
            ...
            if ($group_row) {
                $group_box_has_group = true;
                $group_box_focus_group_id = (int)($group_row['id'] ?? 0);
                ...
                $group_box_member_pending = max(0, (int)($group_row['pending_member_count'] ?? 0));
                $group_box_group_total = max(0, (int)($group_row['total_group_count'] ?? 0));

                $team_status = strtolower(trim((string)($group_row['group_status'] ?? 'active')));
                $group_box_team_status = in_array($team_status, ['active', 'suspended'], true)
                    ? $team_status
                    : 'inactive';

                $group_box_created_at_display = formatSalesManagerDashboardDateTime($group_row['created_at'] ?? '');
                $group_box_updated_at_display = formatSalesManagerDashboardDateTime($group_row['updated_at'] ?? '');
            }
        }
    }
...
$group_box_team_status_class = in_array($group_box_team_status, ['active', 'suspended'], true)
    ...
    : 'inactive';
$group_box_member_count_text = $group_box_has_group
    ? number_format($group_box_member_active) . '/' . number_format($group_box_member_total) . ' คน'
    : '-';
$group_box_group_count_text = $group_box_has_group
    ? number_format(max(1, $group_box_group_total)) . ' ทีม'
    : '0 ทีม';
$group_box_pending_count_text = $group_box_has_group
    ? number_format($group_box_member_pending) . ' รายการ'
    : '0 รายการ';
$group_box_manage_href = 'setup_group_sales.php?module=groupsetup';
if ($group_box_focus_group_id > 0) {
    $group_box_manage_href .= '&group_id=' . rawurlencode((string)$group_box_focus_group_id);
}
                    ...
                    <div class="kpi-inner">
                        ...
                        <?php if ($group_box_has_group): ?>
                            ...
                            <div class="group-box-badges">
                                ...
                                <span class="group-badge info">My Groups: <?php echo htmlspecialchars($group_box_group_count_text); ?></span>
                                <?php if ($group_box_member_pending > 0): ?>
                                    <span class="group-badge pending">Pending: <?php echo htmlspecialchars($group_box_pending_count_text); ?></span>
                                <?php endif; ?>
                            </div>
                            ...
                            <div class="group-box-row">คำขอรออนุมัติ : <strong><?php echo htmlspecialchars($group_box_pending_count_text); ?></strong></div>
                            <div class="group-box-row">สร้างเมื่อ : <?php echo htmlspecialchars($group_box_created_at_display !== '' ? $group_box_created_at_display : '-'); ?></div>
                            ...
                            <a class="group-box-link" href="<?php echo htmlspecialchars($group_box_manage_href); ?>">ไปหน้าจัดการทีมขาย</a>
                        <?php else: ?>
                            <div class="group-box-empty">ยังไม่มีทีมขายในสาขานี้</div>
                            <div class="kpi-sub">เริ่มสร้างทีมและรหัสเชิญสำหรับฝ่ายขาย</div>
                            <div class="group-box-badges">
                                <span class="group-badge inactive">Team: ยังไม่สร้างทีม</span>
                            </div>
                            <a class="group-box-link" href="setup_group_sales.php?module=groupsetup">ไปหน้าสร้างทีมขาย</a>
                        <?php endif; ?>
                    </div>
                </div>
